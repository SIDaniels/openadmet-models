#!/usr/bin/env python
"""
Add SMILES to ToxCast ER/AR assay data using PubChem API.
...
def process_file(input_path, output_path, batch_size=100):
    """Add SMILES to a ToxCast CSV file."""
...
if __name__ == "__main__":
    ...
    # Process ER assays
    er_input = data_dir / "toxcast_er_assays.csv"
    er_output = data_dir / "toxcast_er_assays_smiles.csv"
    process_file(er_input, er_output)

    # Process AR assays
    ar_input = data_dir / "toxcast_ar_assays.csv"
    ar_output = data_dir / "toxcast_ar_assays_smiles.csv"
    process_file(ar_input, ar_output)
