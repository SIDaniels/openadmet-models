#!/usr/bin/env python
"""
Train Estrogen Receptor (ER) binding model using OpenADMET.

Uses ToxCast ER assay data with hitcall as binary classification target.
...
def load_er_data(er_csv_path):
    """Load ER assay data with SMILES."""
    print(f"Loading ER data from {er_csv_path}...")

    data = []
    with open(er_csv_path, 'r') as f:
...
def train_er_model(er_data, output_dir):
    """Train ER binding model."""
    print("\nTraining ER binding model...")

    smiles_list = [d['smiles'] for d in er_data]
    labels = [d['label'] for d in er_data]
    ...
    model_path = output_dir / "er_model.pkl"
    ...
    with open(output_dir / "er_model_metadata.pkl", 'wb') as f:
...
if __name__ == "__main__":
    ...
    er_csv = data_dir / "toxcast_er_assays_smiles.csv"
    output_dir = Path.home() / "openadmet-models" / "models" / "er_model"

    if not er_csv.exists():
        print(f"ERROR: {er_csv} not found!")
        print("Run add_smiles_to_toxcast.py first to get SMILES.")
        exit(1)

    # Load data
    er_data = load_er_data(er_csv)

    # Train model
    model, metadata = train_er_model(er_data, output_dir)

    print("\n" + "=" * 60)
    print("ER Model Training Complete!")
    print(f"Model: {output_dir / 'er_model.pkl'}")
    print(f"Metadata: {output_dir / 'er_model_metadata.pkl'}")
